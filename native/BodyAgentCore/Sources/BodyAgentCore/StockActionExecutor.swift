...
enum StockActionSupport {
    ...
    static func confinedURL(root: URL, relativePath: String, allowRoot: Bool) throws -> URL {
        ...
        guard standardizedCandidate.path.hasPrefix(rootPath) else {
            throw BodyActionError.invalidArguments("BODY_ACTION_PATH_ESCAPE_FORBIDDEN")
        }
...
struct StockBodyActionExecutor: BodyActionExecutor {
    ...
    private func executeFiles(_ request: BodyActionRequest) async throws -> BodyActionResult {
        ...
        case "list":
            ...
            let directory = try StockActionSupport.confinedURL(
                root: root,
        ...
        case "read_text":
            ...
            let file = try StockActionSupport.confinedURL(
                root: root,
                relativePath: relativePath,
                allowRoot: false
            )
        ...
        case "write_text":
            try StockActionSupport.rejectUnexpectedArguments(
                request.arguments,
                allowed: ["path", "text", "overwrite"]
            )
            let relativePath = try StockActionSupport.requiredString(
                request.arguments,
                key: "path",
            ...
            let overwrite = try StockActionSupport.optionalBool(
                request.arguments,
                key: "overwrite"
            )
            let file = try StockActionSupport.confinedURL(
        ...
        case "create_directory":
            try StockActionSupport.rejectUnexpectedArguments(
                request.arguments,
                allowed: ["path", "with_intermediates"]
            )
            let relativePath = try StockActionSupport.requiredString(
                request.arguments,
                key: "path",
            ...
            let intermediates = try StockActionSupport.optionalBool(
                request.arguments,
                key: "with_intermediates"
            )
            let directory = try StockActionSupport.confinedURL(
                root: root,
                relativePath: relativePath,
    ...
    private func listDirectory(_ request: BodyActionRequest, directory: URL) throws -> BodyActionResult {
        ...
        let entries: [JSONValue] = values.prefix(StockActionLimits.listEntries).map { url in
            let resource = try? url.resourceValues(forKeys: keys)
            let kind = resource?.isDirectory == true
                ? "directory"
                : (resource?.isRegularFile == true ? "file" : "other")
            return .object([
    ...
    private func writeText(
        ...
        guard data.count <= StockActionLimits.fileTextBytes else {
            throw BodyActionError.invalidArguments("BODY_ACTION_ARGUMENT_TEXT_INVALID")
        }
    ...
    private func deleteFile(
        ...
        let resource: URLResourceValues
        do {
            resource = try file.resourceValues(forKeys: [.isRegularFileKey])
        } catch {
            throw BodyActionError.executionFailed("BODY_ACTION_FILE_DELETE_FAILED")
        }
        guard resource.isRegularFile == true else {
            throw BodyActionError.executionFailed("BODY_ACTION_DELETE_FILE_ONLY")
        }
