...
public struct RuntimePairingDeviceDescriptor: Codable, Sendable, Equatable {
    ...
    public static func nativeBody(
        id: String,
        label: String = "eCompanion iPhone"
...
public actor RuntimePairingClient {
    ...
    public func claim(
        code: String,
        device: RuntimePairingDeviceDescriptor
        ...
        request.httpBody = try encoder.encode(
            RuntimePairingRequest(code: normalizedCode, device: descriptor)
        )
        ...
        guard (200...299).contains(response.statusCode) else {
            let code = (try? decoder.decode(
                RuntimePairingErrorEnvelope.self,
                from: response.data
            ).error) ?? "HTTP_\(response.statusCode)"
            throw RuntimePairingClientError.runtimeRejected(
        ...
        guard
            let envelope = try? decoder.decode(RuntimePairingEnvelope.self, from: response.data),
            envelope.ok
        else {
            throw RuntimePairingClientError.invalidPayload
        }
        guard !envelope.credential.token.trimmingCharacters(
    ...
    public func enrollmentProfile(
        from result: RuntimePairingResult
