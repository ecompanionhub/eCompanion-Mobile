    ...
    func send(_ request: URLRequest) async throws -> RuntimeHTTPResponse {
        ...
        guard !responses.isEmpty else {
            return RuntimeHTTPResponse(
                statusCode: 500,
                data: Data(#"{"error":"NO_MOCK_RESPONSE"}"#.utf8)
            )
        }
...
final class RuntimePairingClientTests: XCTestCase {
    func testClaimUsesCanonicalPairingEndpointAndNativeDescriptor() async throws {
        ...
        XCTAssertEqual(
            metadata["body_protocol"] as? String,
            "ecompanion-body-ios-v1"
        )
        let capabilities = try XCTUnwrap(
            device["capabilities"] as? [String: Any]
        )
    ...
    func testClaimSurfacesRuntimePairingErrorCode() async throws {
        let transport = PairingRecordingTransport(responses: [
            pairingJSON(
                ["ok": false, "error": "PAIRING_GRANT_INVALID"],
                status: 409
            )
        ])
        let client = try RuntimePairingClient(
    ...
    func testEnrollmentProfileDoesNotPersistBearerCredential() async throws {
        ...
        XCTAssertEqual(profile.credentialID, "credential-1")
